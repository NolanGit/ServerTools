...
class Excel_Data_Controller(object):
    ...
    def get_value(self, excel_file, x = None, y = None):
        # 输入文件地址、行、列，输出值；行、列从0开始计数
        workbook = xlrd.open_workbook(excel_file)
        sheet = workbook.sheet_by_index(0)
        if x == None and y != None:
            return sheet.col_values(int(y))
        if x != None and y == None:
            return sheet.row_values(int(x))
        if x == None and y == None:
            raise ValueError ('There must be at least one coordinate !')
        try:
